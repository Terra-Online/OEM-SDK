...
OEM 和 Game 边界文件统一使用可读的 `{ count, boundaries }` 集合结构。每条边界包含 `id`，polygon `rings` 中的点为 `{ x, z }`；所属 `regionId` 由 manifest 引用确定，不在每个点中重复。游戏关卡网格会按 level 合并后发布，删除共享内边，同时仍可从合并后的环精确还原网格覆盖范围。
