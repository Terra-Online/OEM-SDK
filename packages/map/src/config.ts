  ...
  for (const key of OEM_MAP_CONFIG_KEYS) {
    ...
    switch (OEM_MAP_CONFIG_FIELDS[key].kind) {
      case 'string':
        ...
        break;
      case 'nullableString':
        if (value !== null && typeof value !== 'string') invalid(path, 'Expected a string or null');
        break;
      case 'boolean':
      ...
      case 'number':
        ...
        break;
      case 'theme':
        if (value !== 'light' && value !== 'dark') invalid(path, 'Unknown official theme');
        break;
      case 'boundarySource':
        if (value !== 'oem' && value !== 'game') invalid(path, 'Expected oem or game');
        break;
      case 'center': {
        const center = value as OEMMapConfig['center'];
        if (!center || !Number.isFinite(center.x) || !Number.isFinite(center.z))
          invalid(path, 'Center coordinates must be finite');
        if (center.space !== undefined && center.space !== 'pixel')
          invalid(`${path}.space`, 'Center uses published pixels');
        break;
      }
      case 'markers':
        if (
          ...
          invalid(path, 'Expected type keys, * or false');
        break;
      case 'points': {
        if (!Array.isArray(value)) invalid(path, 'Expected an array');
        value.forEach((point, index) => {
          if (!point || typeof point !== 'object' || !point.position || typeof point.position !== 'object')
            invalid(`${path}[${index}]`, 'Expected a point with a position');
        });
        break;
      }
    }
  }
  if (config.customPoints !== undefined && config.customPointsUrl !== undefined)
    invalid('options.customPoints', 'Pass either customPoints or customPointsUrl, not both');
};
...
export const resolveRegionId = (manifest: OEMManifest, value?: string): string => {
  const requested = value
    ? manifest.regions.some((region) => region.id === value)
      ? value
      : (OEM_REGION_ALIASES[value.toUpperCase()] ?? value)
    : manifest.defaultRegionId;
...
export const normalizeState = (input: OEMMapConfig, manifest: OEMManifest): OEMMapState => {
  ...
  if (subregionId) {
    const owner = manifest.regions.find((region) =>
      region.subregions.some((subregion) => subregion.id === subregionId),
    );
  ...
  const subregion = subregionId ? region.subregions.find((entry) => entry.id === subregionId) : undefined;
  ...
  const requestedLocale =
    input.locale ?? (typeof navigator === 'undefined' ? manifest.fallbackLocale : navigator.language);

  return {
    regionId,
    subregionId,
    floorId,
    locale: normalizeOEMLocale(
      requestedLocale,
...
export function resolveConfigUpdate(
  previous: OEMMapState,
  config: OEMMapConfig,
  manifest: OEMManifest,
): { changes: OEMMapConfig; next: OEMMapState } {
  ...
  if (changes.subregion && changes.region === undefined) {
    const owner = manifest.regions.find((region) =>
      region.subregions.some((sub) => sub.id === changes.subregion),
    );
  ...
  if (changes.subregion !== undefined && changes.subregion !== previous.subregionId) {
    ...
    const preset = getPreset(
      region,
      region.subregions.find((sub) => sub.id === changes.subregion),
    );
    if (changes.center === undefined) merged.center = preset.center;
    if (changes.zoom === undefined) merged.zoom = preset.zoom;
  }
