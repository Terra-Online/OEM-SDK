...
export class OEMMarker extends L.Marker {
  ...
  _animateZoom(event: { center: L.LatLng; zoom: number }): void {
    ...
    if (marker._map)
      marker._setPos(marker._map._latLngToNewLayerPoint(marker._latlng, event.zoom, event.center));
  }
}
...
export class CoveredTileLayer extends SmoothTileLayer {
  constructor(
    ...
    private coverage: OEMRegion['coverage'],
    private floor: OEMFloor,
  ) {
    super(url, options);
  }
  ...
  _isValidTile(coords: L.Coords): boolean {
    ...
    return (
      prototype._isValidTile.call(this, coords) &&
