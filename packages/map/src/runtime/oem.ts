...
const decodeAtlosPoint = (
  raw: AtlosPointTuple | AtlosPointObject,
  region: OEMRegion,
  fallbackSubregionId: string,
): OEMPoint | undefined => {
  if (!Array.isArray(raw) && raw && 'position' in (raw as object) && 'raw' in (raw as object))
    return raw as unknown as OEMPoint;
  ...
  if (typeof x !== 'number' || typeof z !== 'number' || ![x, y, z, tier].every(Number.isFinite))
    return undefined;
...
export class OEM implements OEMContract {
  ...
  private pointMarkers = new Map<
    string,
    { marker: ViewportMarker; inner: HTMLElement; point: OEMPoint; group?: string }
  >();
  ...
  private customMarkers = new Map<
    string,
    { marker: ViewportMarker; point: OEMCustomPoint; inner: HTMLElement }
  >();
  ...
  constructor(
    ...
    private options: OEMOptions,
  ) {
    options.signal?.throwIfAborted();
    if (mounted.has(container)) throw new Error('This container already hosts an OEM instance');
    this.region = getOEMRegion(
      manifest,
      options.view?.regionId ?? options.regionId ?? manifest.defaultRegionId,
    );
    ...
    this.customPoints = new Map(
      this.normalizeCustomPoints(options.customPoints ?? []).map((point) => [point.id, point]),
    );
    ...
    this.resolvedLocale = normalizeOEMLocale(
      this.locale,
  ...
  private assertAlive(): void {
    if (this.destroyed) throw new OEMError('DESTROYED', 'map', 'OEM instance has been destroyed');
  }
  private validateFloor(id: string): void {
    if (!this.region.floors.some((floor) => floor.id === id))
      throw new Error(`Unknown floor ${id} in ${this.region.id}`);
  }
  private validatePosition(position: OEMPosition): void {
  ...
  private normalizeCustomPoints(points: readonly OEMCustomPoint[]): OEMCustomPoint[] {
    ...
    return points.map((point, index) => {
      ...
      if (!Number.isFinite(position.x) || !Number.isFinite(position.z))
        invalid(`${path}.position`, 'Invalid OEM map position');
      if (
        position.subregionId &&
        !pointRegion.subregions.some((subregion) => subregion.id === position.subregionId)
      ) {
  ...
  private loadBoundaryData(reference: OEMAsset): Promise<OEMBoundary[]> {
    return this.repository.load(`boundary:${reference.path}`, async () => {
      const value = await fetchOEMJson<unknown>(
        resolveOEMAsset(this.options.resources.baseUrl, reference.path),
        this.lifetime.signal,
      );
  ...
  async prepareSubregionBoundaries(): Promise<void> {
    ...
    try {
      ...
      this.lifetime.signal.throwIfAborted(); /* Click metadata has a bounds fallback. */
    }
  }
  private inferSubregionId(x: number, z: number): string | undefined {
    const selected = this.filter.subregions?.filter((id) =>
      this.region.subregions.some((subregion) => subregion.id === id),
    );
    ...
    const geometries = this.region.boundaries
      ? (this.repository.peek<OEMBoundary[]>(`boundary:${this.region.boundaries.path}`) ?? [])
      : [];
    const scores = geometries
      .filter(
        (boundary) =>
          this.region.subregions.some((subregion) => subregion.id === boundary.id) &&
  ...
  private capturePosition(x: number, z: number): OEMMapClick {
    ...
    const precise =
      this.region.boundaries &&
      this.repository.peek<OEMBoundary[]>(`boundary:${this.region.boundaries.path}`)?.length;
    const snapshot = createOEMCoordinateSnapshot(
      { regionId: this.region.id, x, z, floorId: this.floorId, ...(subregionId ? { subregionId } : {}) },
      this.manifest,
      subregionId ? (precise ? 'geometry' : 'bounds') : 'unresolved',
    );
  ...
  private emitMapClick = (event: L.LeafletMouseEvent): void => {
    if (!this.destroyed && this.listeners.get('click')?.size)
      this.emit('click', this.capturePosition(event.latlng.lng, event.latlng.lat));
  };
  ...
  private pointInteraction(
    element: HTMLElement,
    trigger: 'pointer' | 'keyboard',
  ): OEMPointInteraction | undefined {
    ...
    if (source === 'custom') {
      ...
      return point
        ? { source, point, trigger, coordinates: createOEMCoordinateSnapshot(point.position, this.manifest) }
        : undefined;
    }
    ...
    return point
      ? {
          ...
          point,
          trigger,
          coordinates: createOEMCoordinateSnapshot(
            toOEMMapPosition({ ...point.position, subregionId: point.subregionId }, this.region),
            this.manifest,
          ),
        }
      : undefined;
  ...
  private keyPoint = (event: KeyboardEvent): void => {
    const element = this.pointElement(event.target);
    if (!element || (event.key !== ' ' && (event.key !== 'Enter' || element instanceof HTMLAnchorElement)))
      return;
  ...
  private emit<Event extends keyof OEMEvents>(event: Event, payload: OEMEvents[Event]): void {
    ...
    const deliver = (handler: (payload: never) => void) => {
      try {
        ...
        const error = new OEMError('CALLBACK_FAILED', event, 'OEM event handler failed', undefined, {
          cause,
        });
  ...
  private emitView = (): void => {
    ...
    if (
      this.emittedView &&
      view.regionId === this.emittedView.regionId &&
      view.floorId === this.emittedView.floorId &&
      view.x === this.emittedView.x &&
      view.z === this.emittedView.z &&
      view.zoom === this.emittedView.zoom
    )
      return;
  ...
  private updateAttribution(): void {
    const messages =
      this.manifest.controls[this.resolvedLocale] ?? this.manifest.controls[this.manifest.fallbackLocale];
  ...
  on<Event extends keyof OEMEvents>(event: Event, handler: (payload: OEMEvents[Event]) => void): () => void {
    this.assertAlive();
    if (event === 'click' || event === 'pointclick' || event === 'pointenter' || event === 'pointleave')
      this.enableCustomInteraction();
  ...
  private applyRegion(view?: OEMView): void {
    ...
    this.map.setView(toOEMLeafletPosition(target, this.region), this.clampZoom(target.zoom), {
      animate: false,
    });
  ...
  private regionBounds(): L.LatLngBounds {
    ...
    return L.latLngBounds(
      ...
      toOEMLeafletPosition(
        { regionId: this.region.id, x: x + this.region.dimensions[0], z: z + this.region.dimensions[1] },
        this.region,
      ),
    );
  }
  ...
  private makeTiles(floorId: string): L.TileLayer {
    ...
    const layer = new CoveredTileLayer(
      resolveOEMAsset(this.options.resources.baseUrl, floor.tileTemplate),
      {
    ...
    layer.on('tileerror', () => {
      if (this.region.id === regionId && this.floorId === floorId)
        this.emit('error', resourceError('tiles', new Error(`Tile load failed: ${regionId}/${floorId}`)));
    });
    return layer;
  }
  ...
  private applyBounds(bounds: [OEMPosition, OEMPosition]): void {
    ...
    this.map.fitBounds(
      L.latLngBounds(
        toOEMLeafletPosition(bounds[0], this.region),
        toOEMLeafletPosition(bounds[1], this.region),
      ),
      { animate: false },
    );
  }
  ...
  private applyLocale(locale: string): void {
    this.assertAlive();
    const resolved = normalizeOEMLocale(
      locale,
      Object.keys(this.manifest.locales),
      this.manifest.fallbackLocale,
    );
  ...
  private setResourceStatus(
    feature: OEMFeatureName,
    status: OEMResourceState['status'],
    error?: Error,
  ): void {
    if (this.destroyed) return;
    this.resourceStates[feature] = {
      requested: !!this.features[feature],
      status,
      ...(error ? { error } : {}),
    };
  ...
  private async applyRetry(feature?: OEMFeatureName): Promise<void> {
    this.assertAlive();
    if (feature !== undefined && !FEATURE_NAMES.includes(feature))
      invalid('retry.feature', 'Unknown feature');
    await Promise.allSettled(
      FEATURE_NAMES.filter(
        (name) =>
          (!feature || name === feature) &&
          this.features[name] &&
          this.resourceStates[name].status === 'error',
      ).map((name) => this.loadFeature(name)),
    );
    this.lifetime.signal.throwIfAborted();
  }
  ...
  private async applyFeatures(
    ...
    reload: readonly OEMFeatureName[] = [],
    retryFailed: readonly OEMFeatureName[] = FEATURE_NAMES,
  ): Promise<void> {
    this.assertAlive();
    if (!features || typeof features !== 'object' || Array.isArray(features))
      invalid('features', 'Expected an object');
    for (const feature of FEATURE_NAMES)
      if (features[feature] !== undefined && typeof features[feature] !== 'boolean')
        invalid(`features.${feature}`, 'Expected boolean');
    if (
      features.boundarySource !== undefined &&
      features.boundarySource !== 'oem' &&
      features.boundarySource !== 'game'
    )
      invalid('features.boundarySource', 'Unknown OEM boundary source');
    const boundaryChanged =
      features.boundarySource !== undefined && features.boundarySource !== this.boundarySource;
    ...
    for (const feature of FEATURE_NAMES) {
      ...
      if (
        requested &&
        (requested !== previous ||
          reload.includes(feature) ||
          (feature === 'boundaries' && boundaryChanged) ||
          (retryFailed.includes(feature) && this.resourceStates[feature].status === 'error'))
      ) {
  ...
  private async readCustomPoints(url: string, signal: AbortSignal): Promise<OEMCustomPoint[]> {
    ...
    try {
      ...
      return invalid('customPointsUrl', 'Invalid URL');
    }
    ...
    return this.normalizeCustomPoints(
      value.map((point) => ({
        ...point,
  ...
  private replaceCustomPoints(points: readonly OEMCustomPoint[]): void {
    ...
    this.renderCustomPoints();
    if (this.listeners.get('custompointschange')?.size)
      this.emit('custompointschange', {
  ...
  async upsertCustomPoints(points: readonly OEMCustomPoint[], options?: OEMCommandOptions): Promise<void> {
    ...
    return this.enqueue(() => {
      ...
      for (const point of snapshot) {
        ...
        this.customPoints.set(point.id, point);
        this.syncCustomPoint(point.id);
      }
  ...
  loadCustomPoints(url: string, options?: OEMCommandOptions): Promise<void> {
    return this.enqueue(
      async (signal) => this.replaceCustomPoints(await this.readCustomPoints(url, signal)),
      options?.signal,
    );
  }
  clearCustomPoints(): Promise<void> {
    return this.enqueue(() => this.replaceCustomPoints([]));
  }
  ...
  private async loadPointIndex(): Promise<Record<string, string>> {
    ...
    return this.repository.load(`index:${reference.path}`, async () => {
      ...
      if (!value || typeof value !== 'object' || Array.isArray(value))
        invalid('pointIndex', 'Invalid OEM point index');
  ...
  private decodePointShard(value: unknown[], shardPath: string, region: OEMRegion = this.region): OEMPoint[] {
    ...
    return value
      .map((entry) =>
        decodeAtlosPoint(entry as AtlosPointTuple | AtlosPointObject, region, fallbackSubregionId),
      )
  ...
  private async loadFeature(feature: OEMFeatureName): Promise<void> {
    ...
    const read = <Data>(ref: OEMAsset) =>
      fetchOEMJson<Data>(resolveOEMAsset(this.options.resources.baseUrl, ref?.path), request.signal);
    try {
      if (feature === 'points') {
        const [groups, types] = await Promise.all([
          Promise.all(
            region.points.map(async (ref) => {
          ...
          Object.keys(this.types).length
            ? this.types
            : read<Record<string, OEMPointType>>(this.manifest.types),
        ]);
        ...
        const [labels, messages] = await Promise.all([
          ...
          this.manifest.locales[this.resolvedLocale]
            ? read<OEMLocaleMessages>(this.manifest.locales[this.resolvedLocale])
            : Promise.resolve({}),
        ]);
        if (request.signal.aborted) return;
        this.labels = labels;
        ...
        const reference =
          this.boundarySource === 'game' ? this.region.gameBoundaries : this.region.boundaries;
        const boundaries = reference
          ? await withOEMAbort(this.loadBoundaryData(reference), request.signal)
          : [];
        if (request.signal.aborted) return;
  ...
  private applyPointFilter(filter: OEMPointFilter): void {
    ...
    for (const key of ['types', 'subregions'] as const)
      if (
        filter[key] !== undefined &&
        (!Array.isArray(filter[key]) ||
          filter[key]!.some((value) => typeof value !== 'string' || !value.trim()))
      )
        invalid(`pointFilter.${key}`, 'Expected non-empty strings');
    if (filter.floorOnly !== undefined && typeof filter.floorOnly !== 'boolean')
      invalid('pointFilter.floorOnly', 'Expected boolean');
  ...
  private createCustomPointVisual(point: OEMCustomPoint): HTMLElement {
    ...
    if (this.customInteractionEnabled) {
      inner.setAttribute('data-oem-point', `custom:${point.id}`);
      inner.tabIndex = 0;
  ...
  private enableCustomInteraction(): void {
    ...
    for (const [id, { marker, inner }] of this.customMarkers) {
      ...
      inner.setAttribute('data-oem-point', `custom:${id}`);
      inner.tabIndex = 0;
      inner.setAttribute('role', 'button');
    }
  }
  private syncCustomPoint(id: string): void {
    const point = this.customPoints.get(id),
      existing = this.customMarkers.get(id);
    if (!point || point.position.regionId !== this.region.id) {
      if (existing) {
        this.customPointsLayer.removeLayer(existing.marker);
        this.customMarkers.delete(id);
      }
      return;
    }
    if (existing) {
      if (existing.point.icon !== point.icon || existing.point.style !== point.style) {
        ...
        if (existing.point.position.x !== point.position.x || existing.point.position.z !== point.position.z)
          existing.marker.setLatLng(toOEMLeafletMapPosition(point.position));
        existing.inner.classList.toggle(
          'offLayer',
          !!point.position.floorId && point.position.floorId !== this.floorId,
        );
    ...
    this.customMarkers.set(id, { marker, point, inner });
  }
  ...
  private renderPoints(): void {
    ...
    const detach = (entry: { marker: ViewportMarker; group?: string }) => {
      if (entry.group) {
        const batch = remove.get(entry.group) ?? [];
        batch.push(entry.marker);
        remove.set(entry.group, batch);
    ...
    for (const point of this.points) {
      ...
      const group =
        this.markerClustering && CLUSTER_SUBCATEGORIES.has(type.category.sub) ? type.key : undefined;
      ...
      if (fresh || previousGroup !== group) {
        ...
        if (group) {
          ...
          const batch = add.get(group) ?? [];
          batch.push(entry!.marker);
          add.set(group, batch);
        } else addPlain.push(entry!.marker);
      }
    }
    for (const [id, entry] of this.pointMarkers)
      if (!desired.has(id)) {
        detach(entry);
  ...
  private renderBoundaries(boundaries: OEMBoundary[]): void {
    ...
    for (const boundary of boundaries) {
      const rings = boundary.rings.map((ring) =>
        ring.map((position) => toOEMLeafletPosition({ ...position, regionId: this.region.id }, this.region)),
      );
      L.polygon(rings, {
        color: 'transparent',
        fillOpacity: 0.2,
  ...
  getState(): OEMMapState {
    ...
    return {
      ...
      subregionId:
        this.filter.subregions?.length === 1 &&
        this.region.subregions.some((subregion) => subregion.id === this.filter.subregions![0])
          ? this.filter.subregions[0]
          : null,
      floorId: this.floorId,
      locale: this.resolvedLocale,
      markerTypes: this.features.points ? (this.filter.types ? [...this.filter.types] : '*') : [],
      labels: !!this.features.labels,
      boundaries: !!this.features.boundaries,
      boundarySource: this.boundarySource,
      markerClustering: this.markerClustering,
      zoom: view.zoom,
      center: { x: view.x, z: view.z },
      theme: this.root.dataset.theme as 'light' | 'dark',
      lockDrag: !!this.options.lockDrag,
      lockZoom: !!this.options.lockZoom,
    };
  }
  ...
  async update(update: OEMMapConfig): Promise<void> {
    ...
    return this.enqueue(async (signal) => {
      ...
      const points =
        changes.customPointsUrl !== undefined
          ? await this.readCustomPoints(changes.customPointsUrl, signal)
          : changes.customPoints !== undefined
            ? this.normalizeCustomPoints(changes.customPoints)
            : undefined;
      signal.throwIfAborted();
      const regionChanged = next.regionId !== previous.regionId,
        localeChanged = next.locale !== previous.locale;
      ...
      if (regionChanged || changes.markerTypes !== undefined || changes.subregion !== undefined)
        this.applyPointFilter({
          ...this.filter,
          types:
            changes.markerTypes === undefined
              ? this.filter.types
              : next.markerTypes === '*'
                ? undefined
                : next.markerTypes,
          subregions:
            changes.subregion === undefined && !regionChanged
      ...
      await this.applyFeatures(
        {
          points: changes.markerTypes !== undefined ? hasMarkers(next) : !!this.features.points,
          labels: next.labels,
          boundaries: next.boundaries,
          boundarySource: next.boundarySource,
        },
        regionChanged ? FEATURE_NAMES : localeChanged ? ['labels'] : [],
        FEATURE_NAMES.filter((feature) =>
          feature === 'points'
            ? changes.markerTypes !== undefined
            : feature === 'labels'
              ? changes.labels !== undefined
              : changes.boundaries !== undefined || changes.boundarySource !== undefined,
        ),
      );
    });
  }
  private applyLocks(locks: OEMInteractionLocks): void {
    for (const key of ['lockDrag', 'lockZoom'] as const)
      if (locks[key] !== undefined && typeof locks[key] !== 'boolean') invalid(key, 'Expected boolean');
    const lockDrag = locks.lockDrag ?? !!this.options.lockDrag,
      lockZoom = locks.lockZoom ?? !!this.options.lockZoom;
    if (lockDrag === !!this.options.lockDrag && lockZoom === !!this.options.lockZoom) return;
    this.options.lockDrag = lockDrag;
    this.options.lockZoom = lockZoom;
    this.map.dragging[lockDrag ? 'disable' : 'enable']();
    for (const handler of [this.map.touchZoom, this.map.boxZoom, this.map.keyboard])
      handler[lockZoom ? 'disable' : 'enable']();
    this.wheel.dispose();
    this.wheel = enableSmoothWheelZoom(this.map, {
      enableInertia: true,
      panEnabled: !lockDrag,
  ...
  setSubregion(subregion: string | null): Promise<void> {
    return this.update({ subregion });
  }
  setFloor(floor: string): Promise<void> {
    return this.update({ floor });
  }
  ...
  setInteractionLocks(locks: OEMInteractionLocks): Promise<void> {
    return this.update(locks);
  }
  setMarkerClustering(markerClustering: boolean): Promise<void> {
    return this.update({ markerClustering });
  }
  ...
  async setPointFilter(filter: OEMPointFilter): Promise<void> {
    ...
    for (const key of ['types', 'subregions'] as const)
      if (filter[key] !== undefined && !Array.isArray(filter[key]))
        invalid(`pointFilter.${key}`, 'Expected an array');
    const snapshot = cloneFilter(filter);
    return this.enqueue(() => this.applyPointFilter(snapshot));
  }
  async setView(view: OEMView): Promise<void> {
    if (!view) invalid('view', 'Expected a view');
    return this.update({
      region: view.regionId,
      floor: view.floorId,
      center: { x: view.x, z: view.z },
      zoom: view.zoom,
    });
  ...
  setZoom(zoom: number, options: OEMZoomOptions = {}): Promise<void> {
    ...
    return this.enqueue(() => this.applyZoom(zoom, snapshot));
  }
  async fitBounds(bounds: [OEMPosition, OEMPosition]): Promise<void> {
    if (!Array.isArray(bounds) || bounds.length !== 2 || bounds.some((point) => !point))
      invalid('bounds', 'Expected two pixel positions');
    const snapshot = bounds.map((point) => ({ ...point })) as [OEMPosition, OEMPosition];
    return this.enqueue(() => this.applyBounds(snapshot));
  }
  async setFeatures(features: OEMFeatures): Promise<void> {
    if (!features || typeof features !== 'object') invalid('features', 'Expected features');
    const snapshot = { ...features };
    return this.enqueue(() => this.applyFeatures(snapshot));
  }
  retry(feature?: OEMFeatureName): Promise<void> {
    return this.enqueue(() => this.applyRetry(feature));
  }
  ...
  unproject(position: OEMScreenPosition): OEMMapClick {
    ...
    if (!Number.isFinite(position.x) || !Number.isFinite(position.y))
      invalid('screenPosition', 'Expected finite CSS pixels');
  ...
  destroy = (): void => {
    ...
    this.markerShells.clear();
    this.markerVisualTemplates = new WeakMap();
