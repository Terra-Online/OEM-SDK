...
const methods: Record<Exclude<keyof OEMMapAPI, 'manifest' | 'destroyed'>, true> = {
  ...
  setTheme: true,
  setFeatures: true,
  getResourceState: true,
  retry: true,
  setPointFilter: true,
  getPointFilter: true,
  setMarkerClustering: true,
  setInteractionLocks: true,
  getCustomPoints: true,
  getCustomPoint: true,
  setCustomPoints: true,
  upsertCustomPoints: true,
  removeCustomPoints: true,
  loadCustomPoints: true,
  clearCustomPoints: true,
  getPoint: true,
  loadPoint: true,
  project: true,
  unproject: true,
...
export function createMapAPI(instance: OEMMapAPI): OEMMapAPI {
  const api = Object.fromEntries(
    (Object.keys(methods) as (keyof typeof methods)[]).map((key) => [key, instance[key].bind(instance)]),
  );
