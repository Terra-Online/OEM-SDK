...
const loadRuntime = (): Promise<OEMRuntime> =>
  (runtimePromise ??= import('./runtime').catch((error) => {
...
export async function createOEM(container: string | HTMLElement, options: OEMOptions): Promise<OEM> {
  ...
  const [manifest, { OEM: OEMClass }] = await withOEMAbort(
    Promise.all([
      options.manifest !== undefined
        ? checkOEMManifestVersion(options.manifest)
        : loadOEMManifest(options.resources, options.signal),
      loadRuntime(),
    ]),
    options.signal,
  );
