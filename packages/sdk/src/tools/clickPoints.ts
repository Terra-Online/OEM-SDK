...
export function createClickPointTool(api: OEMMapAPI, options: OEMClickPointOptions): OEMClickPointTool {
  const validMode = (mode: unknown) => {
    if (mode !== 'single' && mode !== 'multiple') invalid('mode', 'Expected single or multiple');
  };
  validMode(options.mode);
  if (options.style !== 'framed' && options.style !== 'no-frame')
    invalid('style', 'Expected an official point style');
  ...
  const getPoints = () =>
    api.destroyed
      ? []
      : [...ids].flatMap((id) => {
          const point = api.getCustomPoint(id);
          return point ? [point] : [];
        });
  const click = (event: OEMMapClick) => {
    ...
    void command(async () => {
      ...
      await api.upsertCustomPoints([{ id, position, style: config.style, icon: config.icon }], {
  ...
  const unsubscribeChanges = api.on('custompointschange', (event) =>
    event.removed.forEach((id) => ids.delete(id)),
  );
  const destroy = () => {
    if (destroyed) return;
    destroyed = true;
