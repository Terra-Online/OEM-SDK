...
const isTransform = (value: unknown): boolean =>
  value === undefined ||
  (isObject(value) &&
    ['scaleX', 'scaleZ', 'offsetX', 'offsetZ'].every((key) => Number.isFinite(value[key])) &&
    value.scaleX !== 0 &&
    value.scaleZ !== 0);
...
export function validateOEMManifest(value: unknown): OEMManifest {
  ...
  for (const [index, region] of value.regions.entries()) {
    ...
    ids.add(region.id);
    if (!isPair(region.dimensions) || region.dimensions.some((n) => n <= 0))
      invalid(`${path}.dimensions`, 'Expected two positive dimensions');
    if (
      !isObject(region.boundsOffset) ||
      !Number.isFinite(region.boundsOffset.x) ||
      !Number.isFinite(region.boundsOffset.z)
    )
      invalid(`${path}.boundsOffset`, 'Expected finite x/z offsets');
    if (!Number.isFinite(region.tileSize) || (region.tileSize as number) <= 0)
      invalid(`${path}.tileSize`, 'Expected a positive tile size');
    const { minZoom, maxZoom, maxNativeZoom } = region;
    if (
      ![minZoom, maxZoom, maxNativeZoom].every(Number.isFinite) ||
      (minZoom as number) > (maxZoom as number) ||
      (maxNativeZoom as number) > (maxZoom as number) ||
      !Number.isFinite(2 ** (maxNativeZoom as number)) ||
      2 ** (maxNativeZoom as number) === 0
    )
      invalid(path, 'Invalid zoom range');
    if (
      !isObject(region.initialView) ||
      region.initialView.regionId !== region.id ||
      !['x', 'z', 'zoom'].every((key) => Number.isFinite((region.initialView as RecordValue)[key]))
    )
      invalid(`${path}.initialView`, 'Expected a finite view in this region');
    if (!Array.isArray(region.floors) || !region.floors.some((f) => isObject(f) && f.id === 'M'))
      invalid(`${path}.floors`, 'Main floor M is required');
    const floors = new Set<string>();
    for (const [i, floor] of region.floors.entries()) {
      if (!isObject(floor) || !isText(floor.id) || !isText(floor.tileTemplate))
        invalid(`${path}.floors[${i}]`, 'Expected floor ID and tile template');
    ...
    for (const [i, subregion] of region.subregions.entries()) {
      ...
      if (!isObject(subregion) || !isText(subregion.id) || !isText(subregion.key))
        invalid(subpath, 'Expected subregion ID and key');
      if (subregions.has(subregion.id)) invalid(`${subpath}.id`, 'Duplicate subregion');
      subregions.add(subregion.id);
      if (
        subregion.bounds !== undefined &&
        (!Array.isArray(subregion.bounds) || subregion.bounds.length !== 2 || !subregion.bounds.every(isPair))
      )
        invalid(`${subpath}.bounds`, 'Expected two finite corners');
      if (!isTransform(subregion.gameTransform))
        invalid(`${subpath}.gameTransform`, 'Invalid coordinate transform');
    }
  ...
  if (!isObject(value.types) || !isText(value.types.path))
    invalid('manifest.types', 'Missing point type resource');
  if (!isObject(value.locales)) invalid('manifest.locales', 'Expected locale resources');
  if (
    !isText(value.fallbackLocale) ||
    !isObject(value.controls) ||
    !Object.hasOwn(value.controls, value.fallbackLocale)
  )
    invalid('manifest.controls', 'Fallback control messages are unavailable');
  const controls = value.controls[value.fallbackLocale];
  if (
    !isObject(controls) ||
    !['layerSelect', 'zoomIn', 'zoomOut', 'brandName', 'termsOfService'].every((key) => isText(controls[key]))
  )
    invalid('manifest.controls', 'Fallback control messages are unavailable');
  if (value.fonts !== undefined && !Array.isArray(value.fonts))
    invalid('manifest.fonts', 'Expected font resources');
  return value as unknown as OEMManifest;
}
