...
export function createOEMCoordinateSnapshot(
  ...
  subregionResolution: OEMCoordinateSnapshot['subregionResolution'] = position.subregionId
    ? 'provided'
    : 'unresolved',
): OEMCoordinateSnapshot {
  const region = getOEMRegion(manifest, position.regionId);
  const subregion = position.subregionId
    ? region.subregions.find((item) => item.id === position.subregionId)
    : undefined;
  ...
  const gameResolution =
    needsSubregion && (!subregion || !reliable)
      ? 'subregion-unresolved'
      : transform
        ? 'resolved'
  ...
  return Object.freeze({
    ...
    gamePosition:
      gameResolution === 'resolved'
        ? Object.freeze({ ...mapToGameXZPosition(position, region), space: 'game' as const })
        : null,
