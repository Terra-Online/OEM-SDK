...
export function resolveOEMAsset(base: string, assetPath: string): string {
  if (typeof base !== 'string' || typeof assetPath !== 'string' || !assetPath.trim())
    invalid('resource.path', 'Expected a relative resource path');
  // Reject literal URL whitespace/control characters before browser normalization.
  if (
    /^[a-z][a-z\d+.-]*:/i.test(assetPath) ||
    assetPath.startsWith('//') ||
    // eslint-disable-next-line no-control-regex
    /[\\\x00-\x20]/.test(assetPath)
  ) {
  ...
  try {
    ...
    return invalid('resource.path', 'Invalid URL encoding');
  }
  if (decoded.split(/[/?#]/).includes('..') || decoded.includes('\\'))
    invalid('resource.path', 'Resource traversal is not allowed');
  let origin: URL;
  try {
    origin = new URL(base);
  } catch {
    return invalid('resources.baseUrl', 'Expected an absolute HTTP(S) resource base');
  }
  if (
    ...
    invalid('resources.baseUrl', 'Expected an HTTP(S) resource base');
