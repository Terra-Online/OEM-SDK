...
export async function compileOEMStyles(root) {
  ...
  leafletCss = leafletCss.replace(
    /url\((?:"|')?(images\/[^)'"\s]+)(?:"|')?\)/g,
    (match, filename) => `url(./assets/${path.basename(filename)})`,
  );
  const ownCss = sass.compile(path.join(root, 'packages/map/src/styles/index.scss'), {
    style: 'expanded',
  ...
  css.walkAtRules(/keyframes$/, (rule) => {
    const suffix = rule.params.replace(/(^|[-_])(\w)/g, (match, prefix, character) =>
    ...
    rule.selector = selectorParser((selectors) =>
      selectors.each((selector) => {
        ...
        selector.walkClasses((node) => {
          if (node.value === 'leaflet-container') {
            node.value = 'mapRoot';
            rootSelector = true;
          }
        });
