...
const server = createServer(async (request, response) => {
  ...
  try {
    if (pathname === '/') {
      ...
      return response.end(
        '<!doctype html><link rel="stylesheet" href="/style.css"><style>body{margin:0}#host{width:960px;height:640px}</style><div id="host"></div><script src="/app.js"></script>',
      );
    }
    if (pathname === '/app.js') {
      response.setHeader('Content-Type', 'text/javascript');
      return response.end(bundle.outputFiles[0].text);
    }
    if (pathname === '/VL_1.json') return json([['2100500004', 500, 550, 0, 0, 'crate_i']]);
    if (pathname === '/types.json')
      return json({ crate_i: { key: 'crate_i', icon: '/pin.svg', category: { main: 'item', sub: 'item' } } });
    if (pathname === '/pin.svg') {
      response.setHeader('Content-Type', 'image/svg+xml');
      return response.end(
        '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><path fill="#777" d="M0 0h32v32H0z"/></svg>',
      );
    }
    ...
    response.setHeader(
      'Content-Type',
      file.endsWith('.css') ? 'text/css' : file.endsWith('.svg') ? 'image/svg+xml' : 'image/webp',
    );
...
await context.route('https://oem.re/**', (route) =>
  route.fulfill({ contentType: 'text/html', body: '<p>Navigation test</p>' }),
);
...
try {
  ...
  assert.equal(unwantedPopup, false);
  ...
  const interaction = await page.evaluate(() => ({
    clicks: window.clicks.length,
    activations: window.activations,
    enters: window.enters,
  }));
  assert.equal(interaction.clicks, 0);
  assert(interaction.enters.includes('host'));
  ...
  const geometry = await custom.evaluate((element) => {
    const icon = element.closest('.frameMarkerIcon');
    return {
      width: icon.style.width,
      height: icon.style.height,
      left: icon.style.marginLeft,
      top: icon.style.marginTop,
    };
  });
  ...
  await page.evaluate(() => window.api.setZoom(3));
  assert.equal(await page.evaluate(() => window.widget.getState().zoom), 3);
  ...
  assert.equal(record.mapPosition.space, 'map');
  assert.equal(record.pixelPosition.space, 'pixel');
  ...
  await page.evaluate(() => window.api.destroy());
  assert.equal(await page.locator('.oemWidget').count(), 0);
  assert.deepEqual(errors, []);
  console.log(
    'Browser checks passed: default/cancelled navigation, pointer/keyboard events, fixed icon dimensions, shared locks/state, tool ownership and disposal, coordinate persistence.',
  );
} catch (error) {
  console.error(
    await page.evaluate(() => ({
      resources: Object.fromEntries(
        Object.entries(window.api.getResourceState()).map(([k, v]) => [
          k,
          { status: v.status, error: v.error?.message },
        ]),
      ),
      activations: window.activations,
      clicks: window.clicks,
      anchors: [...document.querySelectorAll('[data-oem-point]')].map((e) => ({
        tag: e.tagName,
        href: e.href,
        target: e.target,
