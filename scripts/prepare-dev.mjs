...
const hasPreparedData = async () => {
  try {
    ...
    const hasPlane = (value) =>
      value && Number.isFinite(value.x) && Number.isFinite(value.z) && !('y' in value);
    const hasTransform = (value) =>
      value &&
      [value.scaleX, value.scaleZ, value.offsetX, value.offsetZ].every(Number.isFinite) &&
      value.scaleX !== 0 &&
      value.scaleZ !== 0;
    const references = [
      manifest.types,
    ...
    return (
      ...
      manifest.regions.every(
        (region) =>
          hasPlane(region.boundsOffset) &&
          hasPlane(region.initialView) &&
          hasTransform(region.gameTransform) &&
          typeof region.gameBoundaries?.path === 'string' &&
          Array.isArray(region.subregions) &&
          region.subregions.every(
            (subregion) => !subregion.gameTransform || hasTransform(subregion.gameTransform),
          ),
      ) &&
...
if (!(await hasPreparedData())) {
  ...
  await new Promise((resolvePromise, reject) => {
    const child = spawn(
      process.execPath,
      [path.join(root, 'scripts/export-atlos.mjs'), '--include-licensed-novecento'],
      {
        cwd: root,
        stdio: 'inherit',
      },
