...
const cases: [string, (m: OEMManifest) => void][] = [
  [
    'dimensions',
    (m) => {
      m.regions[0].dimensions = [] as never;
    },
  ],
  [
    'tileSize',
    (m) => {
      m.regions[0].tileSize = Infinity;
    },
  ],
  [
    'zoom range',
    (m) => {
      m.regions[0].minZoom = 8;
    },
  ],
  [
    'locales',
    (m) => {
      delete (m as Partial<OEMManifest>).locales;
    },
  ],
  [
    'boundsOffset',
    (m) => {
      m.regions[0].boundsOffset = undefined as never;
    },
  ],
  [
    'initialView.zoom',
    (m) => {
      m.regions[0].initialView.zoom = NaN;
    },
  ],
  [
    'region object',
    (m) => {
      m.regions[0] = null as never;
    },
  ],
  [
    'duplicate floor',
    (m) => {
      m.regions[0].floors.push(m.regions[0].floors[0]);
    },
  ],
  [
    'duplicate subregion',
    (m) => {
      m.regions[0].subregions.push(m.regions[0].subregions[0]);
    },
  ],
  [
    'position region',
...
describe('runtime wire validation', () => {
  it.each(cases)('rejects malformed %s with a field path', (_, mutate) => {
    ...
    mutate(manifest);
    try {
      validateOEMManifest(manifest);
      throw new Error('Accepted malformed fixture');
    } catch (error) {
      expect(error).toBeInstanceOf(OEMError);
      expect(error).toMatchObject({ code: 'INVALID_INPUT', path: expect.stringContaining('manifest.') });
    }
  });
  ...
  it.each([
    ...
    '/safe/%5c../private',
    '/%',
  ])('rejects unsafe relative resource path %s', (path) => {
  ...
  it('preserves tile template placeholders and a configured base prefix', () => {
    expect(resolveOEMAsset('https://data.example/oem/', '/tiles/{z}/{x}/{y}.webp')).toBe(
      'https://data.example/oem/tiles/{z}/{x}/{y}.webp',
    );
  });
  ...
  it('does not scan trusted manifest contents when loading from the resource origin', async () => {
    ...
    const fetch = vi
      ...
      .mockResolvedValue({ ok: true, json: async () => manifest } as Response);
    try {
  ...
  it('reports invalid channel references as configuration errors', async () => {
    ...
    await expect(
      loadOEMManifest({ baseUrl: 'https://data.example', manifestPath: '/channel' }),
    ).rejects.toBeInstanceOf(OEMError);
    fetch.mockRestore();
  });
});
