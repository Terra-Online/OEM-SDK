...
it('keeps one StrictMode instance, updates all dynamic options and uses current event props', async () => {
  ...
  const root = createRoot(host), ref = createRef<Handle>();
  const manifest = createManifest(), first = vi.fn(), second = vi.fn();
  const options = { manifest, labels: false, markerTypes: ['unused'] };
  const ready = deferred<Handle>();
  try {
    await act(async () => { root.render(createElement(StrictMode, null, createElement(OEMWidget, { ref, options: { ...options, markerTypes: [] }, onMapClick: first, onReady: ready.resolve }))); });
    ...
    await act(async () => { root.render(createElement(StrictMode, null, createElement(OEMWidget, { ref, options: { ...options, markerTypes: [] }, onMapClick: second }))); });
    expect(update).not.toHaveBeenCalled();
    await act(async () => { root.render(createElement(StrictMode, null, createElement(OEMWidget, { ref, options: { ...options, markerTypes: [], theme: 'dark', interaction: { lockZoom: true }, controls: { showScaleBar: false } }, onMapClick: second }))); });
...
it('replays props changed during creation and rebuilds resource context only with a new key', async () => {
  ...
  const host = document.createElement('div'); document.body.append(host);
  const root = createRoot(host), ref = createRef<Handle>();
  const manifest = createManifest(), ready = deferred<Handle>();
  const options = { manifest, labels: false, customPointsUrl: '/pending.json' };
  try {
    await act(async () => root.render(createElement(OEMWidget, { key: 'v1', ref, options, onReady: ready.resolve })));
    await act(async () => root.render(createElement(OEMWidget, { key: 'v1', ref, options: { ...options, theme: 'dark', lockDrag: true }, onReady: ready.resolve })));
    await act(async () => { pending.resolve(json([])); await ready.promise; });
    const first = ref.current!;
    expect(first.getState()).toMatchObject({ theme: 'dark', lockDrag: true });
    expect(fetch).toHaveBeenCalledTimes(1);
    const nextReady = deferred<Handle>();
    await act(async () => root.render(createElement(OEMWidget, { key: 'v2', ref, options: { manifest: createManifest(), labels: false }, onReady: nextReady.resolve })));
    await act(async () => { await nextReady.promise; });
