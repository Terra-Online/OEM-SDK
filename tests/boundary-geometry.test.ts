...
describe('boundary collection protocol', () => {
  it('reads the shared counted collection format', () => {
    expect(parseBoundaryCollection({ count: 1, boundaries: [boundary] }, '/boundaries/test.json')).toEqual([
      boundary,
    ]);
  });
  ...
  it('rejects a collection whose count does not match its boundaries', () => {
    expect(() =>
      parseBoundaryCollection({ count: 2, boundaries: [boundary] }, '/boundaries/bad.json'),
