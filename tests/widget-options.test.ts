...
beforeEach(() => {
  ...
  vi.stubGlobal(
    'fetch',
    vi.fn(async (input) => json(String(input).endsWith('/labels.json') ? [] : {})),
  );
});
afterEach(() => {
  widget?.destroy();
...
describe('Widget uses the shared behavior API', () => {
  ...
  it('hides controls independently', async () => {
    widget = await createOEMWidget(host, {
      manifest: manifest(),
  ...
  it('keeps touch-expanded panels mutually exclusive', async () => {
    ...
    floor.dispatchEvent(touch());
    expect(floor.classList.contains('expanded')).toBe(true);
