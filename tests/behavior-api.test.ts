...
let api: OEMMapAPI;
const point = (id: string, x = 400): OEMCustomPoint => ({
  id,
  position: { regionId: 'Valley_4', x, z: 500, floorId: 'M' },
  style: 'framed',
  icon: '/pin.webp',
});
beforeEach(async () => {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input) => {
      ...
      if (url.endsWith('/types.json'))
        return json({
          crate_i: { key: 'crate_i', icon: '/icon.webp', category: { main: 'item', sub: 'item' } },
        });
      return json({});
    }),
  );
  api = await createOEM(host, {
    resources: { baseUrl: 'https://data.example', manifestPath: '/manifest' },
...
describe('shared public behavior interface', () => {
  it('exposes only the public facade, with no Leaflet or DOM object', () => {
    ...
    expect(api).not.toHaveProperty('map');
    expect(api).not.toHaveProperty('root');
    expect(api).not.toHaveProperty('container');
  });
  it('upserts and removes points without replacing untouched markers', async () => {
    ...
    await api.removeCustomPoints(['b', 'missing']);
    expect(api.getCustomPoints().map((p) => p.id)).toEqual(['a', 'c']);
  ...
  it('provides collection changes and defensive read snapshots', async () => {
    ...
    await api.removeCustomPoints(['a']);
    expect(changed).toHaveBeenLastCalledWith({ added: [], updated: [], removed: ['a'] });
  });
  it('keeps the requested locale while exposing the resolved locale', async () => {
    await api.setLocale('en-GB');
    expect(api.getLocale()).toEqual({ requested: 'en-GB', resolved: 'en-US' });
  });
  it('does not retry an unrelated failed layer when only the view changes', async () => {
    vi.mocked(fetch).mockRejectedValue(new Error('offline'));
    await api.setFeatures({ labels: true });
    vi.mocked(fetch).mockClear();
    await api.setView({ regionId: 'Valley_4', x: 4000, z: 4000, zoom: 3 });
    expect(fetch).not.toHaveBeenCalled();
    expect(api.getResourceState().labels.status).toBe('error');
  });
  ...
  it('resolves a subregion owner and keeps points while switching regions', async () => {
    ...
    manifest.regions.push({
      ...manifest.regions[0],
      id: 'Wuling',
      initialView: { ...manifest.regions[0].initialView, regionId: 'Wuling' },
      subregions: [
        {
          id: 'WL_1',
          key: 'Wuling',
    ...
    await api.upsertCustomPoints([point('a')]);
    await api.setSubregion('WL_1');
    expect(api.getState()).toMatchObject({
      regionId: 'Wuling',
      subregionId: 'WL_1',
      center: { x: 3000, z: 3000 },
    });
    expect(api.getCustomPoint('a')).toEqual(point('a'));
    expect(host.querySelector('.markerInner[aria-label="a"]')).toBeNull();
    await api.setRegion('Valley_4');
    expect(host.querySelector('.markerInner[aria-label="a"]')).not.toBeNull();
  });
});
...
describe('reusable position records', () => {
  ...
  it('does not guess game coordinates when a subregion transform is uncertain', () => {
    ...
    expect(uncertain.gamePosition).toBeNull();
    expect(uncertain.gameResolution).toBe('subregion-unresolved');
    const supplied = createOEMCoordinateSnapshot(position, manifest);
    expect(supplied.gamePosition).toEqual({ space: 'game', x: 150, z: 200 });
    delete manifest.regions[0].gameTransform;
    delete manifest.regions[0].subregions[0].gameTransform;
...
describe('point interaction and composition', () => {
  it('reports custom point activation independently of a map click', async () => {
    ...
    api.on('pointclick', clicked);
    api.on('click', mapClick);
    ...
    expect(clicked).toHaveBeenCalledWith(
      expect.objectContaining({ source: 'custom', trigger: 'pointer', point: point('a') }),
    );
  ...
  it('lets the host cancel published point navigation', async () => {
    ...
    expect(activate).toHaveBeenCalledWith(
      expect.objectContaining({ source: 'published', point: expect.objectContaining({ id: '2100500004' }) }),
    );
  });
  it('tools manage only their own points and preserve them when stopped', async () => {
    ...
    tool.destroy();
    clickMap();
    expect(api.getCustomPoint(owned)).toBeDefined();
    expect(api.getCustomPoint('host')).toBeDefined();
    await tool.clear();
    expect(api.getCustomPoints()).toEqual([point('host')]);
  });
  it('stopping a tool cancels its queued additions without cancelling other commands', async () => {
    ...
    await Promise.resolve();
    tool.destroy();
    pending.resolve(json([point('host')]));
    await load;
    await api.setZoom(3);
    expect(api.getCustomPoints()).toEqual([
      { ...point('host'), icon: new URL('/pin.webp', document.baseURI).href },
    ]);
  });
});
