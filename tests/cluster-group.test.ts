...
beforeEach(() => {
  ...
  group = new CanvasClusterGroup({
    expandOnClick: true,
    disableClusteringAtZoom: 2,
    maxClusterRadius: 60,
    iconCreateFunction: (cluster) =>
      L.divIcon({
        html: `<span class="markerInner"><span class="clusterCount">${cluster.getChildCount()}</span></span>`,
      }),
  });
});
afterEach(() => {
...
const point = (x: number) =>
  new ViewportMarker([0, x], { icon: L.divIcon({ html: '<span class="markerInner">point</span>' }) });
describe('native Canvas cluster group', () => {
  it('separates logical membership from visible summaries and restores all members on zoom', () => {
    const points = [point(0), point(10), point(200)];
    group.addLayers(points).addTo(map);
    ...
    map.setZoom(3);
    expect(points.every((marker) => group.getVisibleParent(marker) === marker && map.hasLayer(marker))).toBe(
      true,
    );
  });
