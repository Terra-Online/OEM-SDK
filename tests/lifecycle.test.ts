...
beforeEach(() => {
  ...
  fetchMock = vi.fn(async (input: RequestInfo | URL) =>
    String(input).endsWith('/labels.json') ? json([]) : json({}),
  );
...
const keep = <T extends { destroy(): void }>(instance: T): T => {
...
describe('creation ownership', () => {
  it('allows an immediate remount after cancelling creation and ignores late cleanup', async () => {
    ...
    await expect(createOEMWidget(host, { manifest: createManifest(), labels: false })).rejects.toThrow(
      'already hosts',
    );
  });
  ...
  it('unmounts React while its manifest request is pending', async () => {
    ...
    const error = vi.fn();
    await act(async () =>
      root.render(createElement(OEMWidget, { options: { resources, onReady: ready, onError: error } })),
    );
    await act(async () => root.unmount());
  ...
  it('does not report cancelled prop updates after React unmounts', async () => {
    ...
    await act(async () =>
      root.render(
        createElement(OEMWidget, { options: { manifest, labels: false, onReady: ready, onError: error } }),
      ),
    );
    await vi.waitFor(() => expect(ready).toHaveBeenCalledOnce());
    const pending = deferred<Response>();
    fetchMock.mockReturnValue(pending.promise);
    await act(async () =>
      root.render(createElement(OEMWidget, { options: { manifest, labels: true, onError: error } })),
    );
  ...
  it('releases a partially constructed renderer after a setup failure', async () => {
    await expect(
      createOEM(host, {
        resources,
        manifest: createManifest(),
        view: { regionId: 'Valley_4', x: 0, z: 0, zoom: NaN },
      }),
    ).rejects.toThrow('Zoom must be finite');
    expect(host.querySelectorAll('.mapRoot')).toHaveLength(0);
...
describe('request lifetime and resource state', () => {
  it.each(['index', 'shard', 'custom'] as const)(
    'aborts a pending %s request on destroy, even without an external signal',
    async (kind) => {
      const map = keep(await createOEM(host, { resources, manifest: createManifest() }));
      const pending = deferred<Response>();
      fetchMock.mockImplementation((input: RequestInfo | URL) => {
        if (kind === 'shard' && String(input).endsWith('/index.json'))
          return Promise.resolve(json({ '2100500004': '/VL_1.json' }));
        return pending.promise;
      });
      const operation =
        kind === 'custom' ? map.loadCustomPoints('/custom.json') : map.loadPoint('2100500004');
  ...
  it('does not wait for boundary metadata during creation and cancels its background request', async () => {
    const manifest = createManifest();
    manifest.regions[0].boundaries = { path: '/boundaries.json', sha256: 'hash', bytes: 2 };
    const pending = deferred<Response>();
    fetchMock.mockReturnValue(pending.promise);
    const controller = new AbortController();
  ...
  it('preserves requested visibility on failure and retries identical Widget configuration', async () => {
    ...
    fetchMock.mockImplementation(async (input) => {
      if (String(input).endsWith('/labels.json')) {
        if (fail) throw new Error('labels unavailable');
        return json([]);
      }
      return json({});
    });
    const error = vi.fn();
    const widget = keep(
      await createOEMWidget(host, { resources, manifest: createManifest(), locale: 'en-US', onError: error }),
    );
    expect(widget.getState().labels).toBe(true);
    expect(widget.getResourceState().labels).toMatchObject({
      requested: true,
      status: 'error',
    ...
    const events = vi.fn();
    widget.on('resourcechange', events);
    fail = false;
    ...
    const snapshot = widget.getResourceState();
    snapshot.labels.status = 'error';
  ...
  it('offers explicit retry and never retries a disabled layer', async () => {
    ...
    const errorSnapshot = map.getResourceState();
    errorSnapshot.labels.error!.message = 'mutated';
    expect(map.getResourceState().labels.error!.message).toBe('unavailable');
    fetchMock.mockImplementation(async (input) =>
      String(input).endsWith('/labels.json') ? json([]) : json({}),
    );
    await map.retry('labels');
    expect(map.getResourceState().labels.status).toBe('ready');
    await map.setFeatures({ labels: false });
    fetchMock.mockClear();
    await map.retry();
  ...
  it('does not treat an observer exception as a failed layer load', async () => {
    const error = vi.fn();
    const map = keep(await createOEM(host, { resources, manifest: createManifest(), onError: error }));
    map.on('resourcechange', () => {
...
describe('configuration failure isolation', () => {
  it('does not commit a view change when required custom point loading fails, then applies the next update', async () => {
    ...
    await rejected;
    expect(widget.getState()).toEqual(before);
  ...
  it('settles optional locale failure without leaving Widget state behind the renderer', async () => {
    const widget = keep(
      await createOEMWidget(host, { resources, manifest: createManifest(), locale: 'en-US' }),
    );
    fetchMock.mockImplementation(async (input) => {
      if (String(input).endsWith('/zh.json')) throw new Error('locale failed');
      return json([]);
    });
  ...
  it('validates the whole patch before changing points or view', async () => {
    ...
    await expect(
      widget.setOptions({
        customPoints: [
          { id: 'valid', style: 'framed', icon: '/icon', position: { regionId: 'Valley_4', x: 1, z: 2 } },
        ],
        zoom: Infinity,
      }),
    ).rejects.toMatchObject({ code: 'INVALID_INPUT', path: 'options.zoom' });
    expect(widget.getState()).toEqual(before);
    expect(host.querySelectorAll('.frameMarkerIcon')).toHaveLength(0);
    await widget.setOptions({ zoom: 3 });
    expect(widget.getState().zoom).toBe(3);
  });
  ...
  it('does not cancel valid in-flight data when a replacement is invalid', async () => {
    ...
    pending.resolve(
      json([{ id: 'valid', style: 'framed', icon: '/icon', position: { regionId: 'Valley_4', x: 1, z: 2 } }]),
    );
    await operation;
